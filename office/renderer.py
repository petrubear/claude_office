...
class Renderer:
    ...
    def draw(self, scene, characters):
        ...
        # 5. Speech bubbles (on top, with color based on state)
        for char in sorted_chars:
            if char.is_alive:
                char.render_bubble(self.stdscr)

        # 6. Status bar
        main_count = sum(1 for c in characters.values()
                         if c.agent_type == "main" and c.is_alive)
        sub_count = sum(1 for c in characters.values()
                        if c.agent_type != "main" and c.is_alive)
        active_count = sum(1 for c in characters.values()
                           if c.state == AgentState.WORKING and c.is_alive)
        tools = [c.current_tool for c in characters.values()
                 if c.current_tool and c.is_alive]
    ...
    def _draw_resize_message(self, max_h, max_w):
        msg = "Resize terminal (80x24 min)"
        y = max_h // 2
        x = max(0, (max_w - len(msg)) // 2)
        try:
            self.stdscr.addstr(y, x, msg[:max_w])
        except curses.error:
            pass
